...
# but there is a catch: he will get there only if you tell him how much that is going to take. 
...
# [5, 3, 4, 6, 1], then [3, 4, 6, 1, 4] and so on. 
# In the end, our buddy will be queuing for 12 minutes, true story!

# Build a function to compute it, 
# resting assured that only positive integers are going to be there and 
# you will be always given a valid index; but we also want to go to pretty popular events, 
...
def queue(queuers,pos):
    ...
    d = dict(zip(range(len(queuers)), queuers))
    order = []
    
    while len(order) < sum(queuers):
        for key, value in d.items():
            if d[key] > 0:
                order.append(key)
                d[key] -= 1
                
    return len(order) - order[::-1].index(pos)