# Kata: Tetris Series #1 — Scoring System
# https://www.codewars.com/kata/5da9af1142d7910001815d32

# A History Lesson

# Tetris is a puzzle video game originally designed and programmed by Soviet Russian software engineer 
# Alexey Pajitnov. The first playable version was completed on June 6, 1984. Pajitnov derived its name 
# from combining the Greek numerical prefix tetra- (the falling pieces contain 4 segments) and tennis, 
...
# For example, a single line clear is worth 40 points, clearing four lines at once (known as
# a Tetris) is worth 1200.

# A level multiplier is also used. The game starts at level 0. 
# The level increases every ten lines you clear. Note that after increasing the level, 
# the total number of cleared lines is not reset.
...
def get_score(arr) -> int:
    ...
    level, level_up_count, total = 0, 0, 0
    
    for lines in arr:

        # if finished line > 10, level up and reset the counter
        if level_up_count >= 10: 
            level += 1
            level_up_count -= 10

        # score = base-score * (level + 1)
        total += (score.get(lines,0))*(level+1)
        level_up_count += lines
