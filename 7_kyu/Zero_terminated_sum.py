...
# This book is filled with zero-terminated substrings, 
# and you have to find the substring with the largest sum of its digits. 
...
# A substring can have length 0. For example, 123004560 has three substrings, 
# and the middle one has length 0.
# All inputs will be valid strings of digits, and the last digit will always be 0.

def largest_sum(s):
    return max(sum(map(int, subStr)) for subStr in s.split('0'))