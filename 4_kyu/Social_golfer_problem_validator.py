...
# A group of N golfers wants to play in groups of G players for D days in such a way 
# that no golfer plays more than once with any other golfer. 
...
# as being a solution to the social golfer problem. Each character represents a golfer, 
...
# (2) that the number and size of the groups is the same every day, and 
# (3) that each player plays with every other player at most once.

# So although each player must play every day, 
# there can be particular pairs of players that never play together.

# It is not necessary to consider the case where the number of golfers is zero; 
# no tests will check for that. If you do wish to consider that case, 
# note that you should accept as valid all possible solutions for zero golfers, 
        ...
        for group in day:
            ...
            for player in group:
                ...
                if player not in d:
                    ...
                    if len(d[player] & set(group)) > 1: # & -> set intersaction
                        return False
